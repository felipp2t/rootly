...
import { canManageInvites } from './can-manage-invites.ts'
...
describe('CanManageInvites', () => {
  ...
  it('should allow a non-owner member with member:invite permission', async () => {
    const userId = new UniqueEntityID().toString()
    const workspaceId = new UniqueEntityID().toString()
    const ownerId = new UniqueEntityID().toString()
    ...
    rolePermissionRepository.items.push(
      RolePermission.create({ roleId, resource: 'member', action: 'invite' }),
    )
  ...
  it('should allow a non-owner member with member:all permission', async () => {
    const userId = new UniqueEntityID().toString()
    const workspaceId = new UniqueEntityID().toString()
    const ownerId = new UniqueEntityID().toString()
    const roleId = new UniqueEntityID().toString()
    ...
    rolePermissionRepository.items.push(
      RolePermission.create({ roleId, resource: 'member', action: 'all' }),
    )
