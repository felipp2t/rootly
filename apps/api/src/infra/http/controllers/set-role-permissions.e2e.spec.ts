...
describe('PUT /workspaces/:workspaceId/roles/:roleId/permissions', () => {
  beforeAll(async () => {
    await app.ready()
  })
  ...
  it('should set permissions for a role and return 204', async () => {
  ...
  it('should replace all existing permissions when called again', async () => {
    const { cookieHeader } = await createUserAndAuthenticate()
    const workspaceId = await createWorkspace(cookieHeader)
    const roleId = await createRole(cookieHeader, workspaceId, 'Editor')
  ...
  it('should accept an empty permissions array and return 204', async () => {
    const { cookieHeader } = await createUserAndAuthenticate()
    const workspaceId = await createWorkspace(cookieHeader)
    const roleId = await createRole(cookieHeader, workspaceId, 'Editor')
